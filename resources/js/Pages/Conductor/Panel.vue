...
const estadisticas = computed(() => {
  ...
  return [
    { label: 'Viajes hoy', value: viajesHoy.length, icon: 'Ã°Å¸Å¡â€¢', change: '+2 ayer' },
    { label: 'Ganancias hoy', value: `${earnings.toFixed(2)} Ã¢â€šÂ¬`, icon: 'Ã°Å¸â€™Â°', change: `media ${(earnings / (viajesHoy.length || 1)).toFixed(2)} Ã¢â€šÂ¬` },
    { label: 'ValoraciÃƒÂ³n', value: conductorStore.formattedRating, icon: 'Ã¢Â­Â', change: 'de 5' },
    { label: 'Total viajes', value: conductorStore.estadisticas.totalviajes, icon: 'Ã°Å¸â€œÅ ', change: 'todo el tiempo' }
  ]
})
...
<template>
  <ConductorLayout>
    <div class="flex justify-between items-start mb-8">
      <div>
        <h2 class="text-2xl font-bold text-neutral-dark">Ã‚Â¡Hola, {{ authStore.usuario?.name }}! Ã°Å¸â€˜â€¹</h2>
        <p class="text-neutral-slate">Panel de control de taxista</p>
      </div>
      <div class="flex items-center space-x-4">
        <span class="text-sm text-neutral-slate">Estado:</span>
        <button
          @click="conductorStore.toggleOnlineStatus()"
          :class="[
            'px-4 py-2 rounded-lg font-semibold transition-colors',
            conductorStore.isOnline ? 'bg-green-500 text-white' : 'bg-neutral-slate text-white'
          ]"
        >
          {{ conductorStore.isOnline ? 'Ã°Å¸Å¸Â¢ En lÃƒÂ­nea' : 'Ã¢Å¡Â« Desconectado' }}
        </button>
      </div>
    </div>
    ...
    <div v-if="conductorStore.isOnline && solicitudesPendientes.length > 0" class="mb-8">
      <h3 class="font-semibold text-neutral-dark mb-4 text-lg">Ã°Å¸â€œâ€¹ Solicitudes pendientes ({{ solicitudesPendientes.length }})</h3>
            ...
            <div>
              ...
              <div class="space-y-2 text-sm">
                <p class="flex items-center text-neutral-slate">
                  <span class="w-16 text-neutral-dark">Ã°Å¸â€œÂ Origen:</span>
                  {{ solicitud.pickup }}
                </p>
                <p class="flex items-center text-neutral-slate">
                  <span class="w-16 text-neutral-dark">Ã°Å¸ÂÂ Destino:</span>
                  {{ solicitud.dropoff }}
                </p>
              </div>
              <div class="flex space-x-4 mt-3 text-xs">
                <span class="bg-neutral-100 px-2 py-1 rounded-full">Ã°Å¸â€œÂ {{ solicitud.distance }} km</span>
                <span class="bg-neutral-100 px-2 py-1 rounded-full">Ã°Å¸â€™Â° {{ solicitud.price }} Ã¢â€šÂ¬</span>
              </div>
            </div>
    ...
    <div v-if="viajesAceptados.length > 0" class="mb-8">
      <h3 class="font-semibold text-neutral-dark mb-4 text-lg">Ã°Å¸Å¡â€¢ Viaje en curso</h3>
        ...
        <table class="w-full">
          <thead class="bg-neutral-soft">
            <tr>
              ...
              <th class="text-left p-4 text-sm font-medium text-neutral-slate">ValoraciÃƒÂ³n</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="viaje in viajesHoy" :key="viaje.id" class="border-b border-neutral-volcanic hover:bg-neutral-soft">
              ...
              <td class="p-4 font-medium text-green-600">{{ viaje.price }} Ã¢â€šÂ¬</td>
              <td class="p-4">
                <div class="flex items-center">
                  <span class="text-yellow-400">Ã¢Ëœâ€¦</span>
                  <span class="ml-1">{{ viaje.valoracion || 'Ã¢â‚¬â€' }}</span>
                </div>
              </td>
            </tr>
          </tbody>
    ...
    <div v-if="conductorStore.perfil" class="mt-8 bg-white rounded-xl shadow-sm p-6">
      <h3 class="font-semibold text-neutral-dark mb-4">Mi vehÃƒÂ­culo</h3>
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div>
          <p class="text-xs text-neutral-slate">MatrÃƒÂ­cula</p>
          <p class="font-medium">{{ conductorStore.perfil.vehicle.plate }}</p>
        </div>
        <div>
          <p class="text-xs text-neutral-slate">AÃƒÂ±o</p>
