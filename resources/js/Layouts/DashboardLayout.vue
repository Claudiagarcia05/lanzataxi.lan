...
const notificaciones = ref([
  ...
  { id: 2, text: 'Viaje completado con ÃƒÂ©xito', time: 'hace 15 min', read: false },
  { id: 3, text: 'Pago recibido: 18.50Ã¢â€šÂ¬', time: 'hace 1 hora', read: true }
])
...
const logout = async () => {
  if (authStore.isconductor && conductorStore.isOnline) {
    conductorStore.toggleOnlineStatus()
  }
  authStore.logout()
  inertiaRouter.visit('/')
}
...
const getDashboardTitle = () => {
  ...
  if (rutaActual.value.includes('/admin')) return 'Panel AdministraciÃƒÂ³n'
        ...
        <div class="flex items-center space-x-3">
          ...
          <div v-if="isSidebarOpen" class="overflow-hidden">
            ...
            <p v-if="authStore.isconductor" class="text-xs mt-1">
              <span :class="conductorStore.isOnline ? 'text-green-500' : 'text-neutral-slate'">
