...
import { useTripStore } from './viajeStore'
...
export const useConductorStore = defineStore('conductor', {
  ...
  getters: {
    ...
    gananciasHoyFormateadas: (state) => `${state.estadisticas.gananciasHoy.toFixed(2)} â‚¬`,
    estaDisponible: (state) => state.estaEnLinea && state.perfil?.verified,
    isOnline: (state) => state.estaEnLinea,
    isAvailable: (state) => state.estaEnLinea && state.perfil?.verified,
    formattedRating: (state) => state.estadisticas.valoracion.toFixed(2),
    todayEarningsFormatted: (state) => `${state.estadisticas.gananciasHoy.toFixed(2)} â‚¬`
  },
  ...
  actions: {
      ...
      try {
        ...
        this.perfil = {
          ...
          vehicle: {
            ...
            plate: taxi.plate || 'Sin matrÃ­cula',
    ...
    toggleOnlineStatus() { return this.cambiarEstadoEnLinea() },
