<template>
  <div v-if="show" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-2xl p-8 max-w-md w-full">
      ...
      <p class="text-sm text-neutral-slate mb-6">Selecciona tu mÃ©todo de pago preferido</p>
      ...
      <div class="bg-neutral-soft p-4 rounded-lg mb-6">
        ...
        <div class="flex justify-between pt-2 border-t border-neutral-volcanic">
          ...
          <span class="text-xl font-bold text-lanzarote-blue">{{ viaje?.price?.toFixed(2) }} â‚¬</span>
        </div>
      </div>
      ...
      <div class="space-y-3 mb-6">
        ...
        >
          ...
          <span v-if="selectedMethod === method.value" class="text-lanzarote-blue">âœ“</span>
        </button>
      </div>

      <!-- Detalles adicionales segÃºn mÃ©todo -->
      <div v-if="selectedMethod === 'card' || selectedMethod === 'stripe'" class="mb-6 space-y-3">
        <div>
          <label class="block text-sm font-medium text-neutral-slate mb-2">NÃºmero de tarjeta</label>
          <input 
...
const pagoMethods = [
  { value: 'cash', label: 'Efectivo', description: 'Pagar al conductor', icon: 'ðŸ’µ' },
  { value: 'card', label: 'Tarjeta', description: 'DÃ©bito o crÃ©dito', icon: 'ðŸ’³' },
  { value: 'stripe', label: 'stripe', description: 'Pago online seguro', icon: 'ðŸ”’' },
  { value: 'paypal', label: 'PayPal', description: 'Cuenta PayPal', icon: 'ðŸ…¿ï¸' }
]
